...
module SwaggerClient
  class ResultsByYearResponseMRData
    attr_accessor :xmlns
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ResultsByYearResponseMRData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ResultsByYearResponseMRData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'race_table')
        self.race_table = attributes[:'race_table']
      end
    end
    ...
    def hash
      [xmlns, series, url, limit, offset, total, race_table].hash
    end
