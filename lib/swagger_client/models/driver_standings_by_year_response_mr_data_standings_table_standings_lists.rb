...
module SwaggerClient
  class DriverStandingsByYearResponseMRDataStandingsTableStandingsLists
    attr_accessor :season

    attr_accessor :round

    attr_accessor :driver_standings
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::DriverStandingsByYearResponseMRDataStandingsTableStandingsLists` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::DriverStandingsByYearResponseMRDataStandingsTableStandingsLists`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'driver_standings')
        if (value = attributes[:'driver_standings']).is_a?(Array)
          self.driver_standings = value
        end
      end
    end
    ...
    def hash
      [season, round, driver_standings].hash
    end
