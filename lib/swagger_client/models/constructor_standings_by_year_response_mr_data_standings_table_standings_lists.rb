...
module SwaggerClient
  class ConstructorStandingsByYearResponseMRDataStandingsTableStandingsLists
    attr_accessor :season

    attr_accessor :round

    attr_accessor :constructor_standings
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ConstructorStandingsByYearResponseMRDataStandingsTableStandingsLists` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ConstructorStandingsByYearResponseMRDataStandingsTableStandingsLists`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'constructor_standings')
        if (value = attributes[:'constructor_standings']).is_a?(Array)
          self.constructor_standings = value
        end
      end
    end
    ...
    def hash
      [season, round, constructor_standings].hash
    end
