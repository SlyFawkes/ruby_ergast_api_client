  ...
  class Circuit
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::Circuit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::Circuit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'circuit_id')
        self.circuit_id = attributes[:'circuit_id']
      end
      ...
      if attributes.key?(:'circuit_name')
        self.circuit_name = attributes[:'circuit_name']
      end
    ...
    def hash
      [circuit_id, round, url, circuit_name, location].hash
    end
